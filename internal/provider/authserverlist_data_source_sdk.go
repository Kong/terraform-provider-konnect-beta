// Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.

package provider

import (
	"context"
	"github.com/hashicorp/terraform-plugin-framework/diag"
	"github.com/hashicorp/terraform-plugin-framework/types"
	"github.com/kong/terraform-provider-konnect-beta/internal/provider/typeconvert"
	tfTypes "github.com/kong/terraform-provider-konnect-beta/internal/provider/types"
	"github.com/kong/terraform-provider-konnect-beta/internal/sdk/models/operations"
	"github.com/kong/terraform-provider-konnect-beta/internal/sdk/models/shared"
)

func (r *AuthServerListDataSourceModel) ToOperationsListAuthServersRequest(ctx context.Context) (*operations.ListAuthServersRequest, diag.Diagnostics) {
	var diags diag.Diagnostics

	pageSize := new(int64)
	if !r.PageSize.IsUnknown() && !r.PageSize.IsNull() {
		*pageSize = r.PageSize.ValueInt64()
	} else {
		pageSize = nil
	}
	pageNumber := new(int64)
	if !r.PageNumber.IsUnknown() && !r.PageNumber.IsNull() {
		*pageNumber = r.PageNumber.ValueInt64()
	} else {
		pageNumber = nil
	}
	out := operations.ListAuthServersRequest{
		PageSize:   pageSize,
		PageNumber: pageNumber,
	}

	return &out, diags
}

func (r *AuthServerListDataSourceModel) RefreshFromSharedAuthServerList(ctx context.Context, resp *shared.AuthServerList) diag.Diagnostics {
	var diags diag.Diagnostics

	if resp != nil {
		r.Data = []tfTypes.AuthServer{}
		if len(r.Data) > len(resp.Data) {
			r.Data = r.Data[:len(resp.Data)]
		}
		for dataCount, dataItem := range resp.Data {
			var data tfTypes.AuthServer
			data.Audience = types.StringValue(dataItem.Audience)
			data.CreatedAt = types.StringValue(typeconvert.TimeToString(dataItem.CreatedAt))
			data.Description = types.StringValue(dataItem.Description)
			data.ID = types.StringValue(dataItem.ID)
			data.Issuer = types.StringValue(dataItem.Issuer)
			if len(dataItem.Labels) > 0 {
				data.Labels = make(map[string]types.String, len(dataItem.Labels))
				for key, value := range dataItem.Labels {
					data.Labels[key] = types.StringPointerValue(value)
				}
			}
			data.MetadataURI = types.StringValue(dataItem.MetadataURI)
			data.Name = types.StringValue(dataItem.Name)
			if dataItem.SigningAlgorithm != nil {
				data.SigningAlgorithm = types.StringValue(string(*dataItem.SigningAlgorithm))
			} else {
				data.SigningAlgorithm = types.StringNull()
			}
			data.UpdatedAt = types.StringValue(typeconvert.TimeToString(dataItem.UpdatedAt))
			if dataCount+1 > len(r.Data) {
				r.Data = append(r.Data, data)
			} else {
				r.Data[dataCount].Audience = data.Audience
				r.Data[dataCount].CreatedAt = data.CreatedAt
				r.Data[dataCount].Description = data.Description
				r.Data[dataCount].ID = data.ID
				r.Data[dataCount].Issuer = data.Issuer
				r.Data[dataCount].Labels = data.Labels
				r.Data[dataCount].MetadataURI = data.MetadataURI
				r.Data[dataCount].Name = data.Name
				r.Data[dataCount].SigningAlgorithm = data.SigningAlgorithm
				r.Data[dataCount].UpdatedAt = data.UpdatedAt
			}
		}
		r.Meta.Page.Number = types.Float64Value(resp.Meta.Page.Number)
		r.Meta.Page.Size = types.Float64Value(resp.Meta.Page.Size)
		r.Meta.Page.Total = types.Float64Value(resp.Meta.Page.Total)
	}

	return diags
}
