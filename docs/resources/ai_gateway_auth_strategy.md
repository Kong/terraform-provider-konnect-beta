---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "konnect_ai_gateway_auth_strategy Resource - terraform-provider-konnect-beta"
subcategory: ""
description: |-
  AIGatewayAuthStrategy Resource
---

# konnect_ai_gateway_auth_strategy (Resource)

AIGatewayAuthStrategy Resource

## Example Usage

```terraform
resource "konnect_ai_gateway_auth_strategy" "my_aigatewayauthstrategy" {
  provider = konnect-beta
  gateway_id = "5f9fd312-a987-4628-b4c5-bb4f4fddd5f7"
  key_auth = {
    config       = "{ \"see\": \"documentation\" }"
    display_name = "Okta AI SE"
    labels = {
      key = "value"
    }
    managed_by = {
      key = "value"
    }
    name = "okta-ai-se"
  }
  openid_connect = {
    config       = "{ \"see\": \"documentation\" }"
    display_name = "Okta AI SE"
    labels = {
      key = "value"
    }
    managed_by = {
      key = "value"
    }
    name = "okta-ai-se"
  }
}
```

<!-- schema generated by tfplugindocs -->
## Schema

### Required

- `gateway_id` (String) The unique ID of the AI Gateway.

### Optional

- `key_auth` (Attributes) **Pre-release Feature**
This feature is currently in beta and is subject to change.

Configuration for an auth strategy. (see [below for nested schema](#nestedatt--key_auth))
- `openid_connect` (Attributes) **Pre-release Feature**
This feature is currently in beta and is subject to change.

Configuration for an auth strategy. (see [below for nested schema](#nestedatt--openid_connect))

### Read-Only

- `config` (String) Configuration for the key-auth auth strategy.
For advanced use cases, additional config properties can be sent in the request body.
See: https://developer.konghq.com/plugins/key-auth/reference/ for the list of properties
Parsed as JSON.
- `created_at` (String) An ISO-8601 timestamp representation of entity creation date.
- `display_name` (String) The display name for this auth strategy instance.
- `id` (String) Contains a unique identifier used for this resource.
- `name` (String) **Pre-release Feature**
This feature is currently in beta and is subject to change.

A user-defined unique identifier for this auth strategy instance, used as a stable human-readable reference. This value is immutable after creation.
- `updated_at` (String) An ISO-8601 timestamp representation of entity update date.

<a id="nestedatt--key_auth"></a>
### Nested Schema for `key_auth`

Optional:

- `config` (String) Configuration for the key-auth auth strategy.
For advanced use cases, additional config properties can be sent in the request body.
See: https://developer.konghq.com/plugins/key-auth/reference/ for the list of properties
Parsed as JSON.
- `display_name` (String) The display name for this auth strategy instance. Not Null
- `labels` (Map of String) Public labels store information about an entity that can be used for filtering a list of objects.

Public labels are intended to store **PUBLIC** metadata. 

Keys must be of length 1-63 characters, and cannot start with "kong", "konnect", "mesh", "kic", or "_".
- `managed_by` (Map of String) Stores information about what manages this entity, such as the tool or system responsible for its lifecycle (for example, `terraform`).

Keys must be 1–63 characters long and start with an alphanumeric character.
- `name` (String) **Pre-release Feature**
This feature is currently in beta and is subject to change.

A user-defined unique identifier for this auth strategy instance, used as a stable human-readable reference. This value is immutable after creation.
Not Null

Read-Only:

- `created_at` (String) An ISO-8601 timestamp representation of entity creation date.
- `id` (String) Contains a unique identifier used for this resource.
- `updated_at` (String) An ISO-8601 timestamp representation of entity update date.


<a id="nestedatt--openid_connect"></a>
### Nested Schema for `openid_connect`

Optional:

- `config` (String) Configuration for the OpenID Connect auth strategy.
For advanced use cases, additional config properties can be sent in the request body.
See: https://developer.konghq.com/plugins/openid-connect/reference/ for the list of properties
Parsed as JSON.
- `display_name` (String) The display name for this auth strategy instance. Not Null
- `labels` (Map of String) Public labels store information about an entity that can be used for filtering a list of objects.

Public labels are intended to store **PUBLIC** metadata. 

Keys must be of length 1-63 characters, and cannot start with "kong", "konnect", "mesh", "kic", or "_".
- `managed_by` (Map of String) Stores information about what manages this entity, such as the tool or system responsible for its lifecycle (for example, `terraform`).

Keys must be 1–63 characters long and start with an alphanumeric character.
- `name` (String) **Pre-release Feature**
This feature is currently in beta and is subject to change.

A user-defined unique identifier for this auth strategy instance, used as a stable human-readable reference. This value is immutable after creation.
Not Null

Read-Only:

- `created_at` (String) An ISO-8601 timestamp representation of entity creation date.
- `id` (String) Contains a unique identifier used for this resource.
- `updated_at` (String) An ISO-8601 timestamp representation of entity update date.

## Import

Import is supported using the following syntax:

In Terraform v1.5.0 and later, the [`import` block](https://developer.hashicorp.com/terraform/language/import) can be used with the `id` attribute, for example:

```terraform
import {
  to = konnect_ai_gateway_auth_strategy.my_konnect_ai_gateway_auth_strategy
  id = jsonencode({
    gateway_id = "5f9fd312-a987-4628-b4c5-bb4f4fddd5f7"
    id         = "5f9fd312-a987-4628-b4c5-bb4f4fddd5f7"
  })
}
```

The [`terraform import` command](https://developer.hashicorp.com/terraform/cli/commands/import) can be used, for example:

```shell
terraform import konnect_ai_gateway_auth_strategy.my_konnect_ai_gateway_auth_strategy '{"gateway_id": "5f9fd312-a987-4628-b4c5-bb4f4fddd5f7", "id": "5f9fd312-a987-4628-b4c5-bb4f4fddd5f7"}'
```
